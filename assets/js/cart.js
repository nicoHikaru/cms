...
if(formCarts !== null) {
    formCarts.forEach((formCart) => {
        
        formCart.addEventListener('click',(event) => {

            event.preventDefault();
            
            async function getCart(){
              let rep = await fetch('/produit/cart/' + formCart.children[1].value + '/' + userId.value , { method: 'GET' });
              let reponse = await rep.json();
      
              if(reponse.cart === true) {
                  let svg = `
                  ...
                  `;
                  formCart.children[0].innerHTML = svg;
              }
              if(reponse.cart === false) {
                  let svg = `
...
if(formCartsHome !== null) {
    formCartsHome.forEach((formCart) => {
        
        formCart.addEventListener('click',(event) => {

            event.preventDefault();
            
            async function getCartHome(){
              let rep = await fetch('/produit/cart/' + formCart.children[1].value + '/' + userId.value , { method: 'GET' });
              let reponse = await rep.json();
